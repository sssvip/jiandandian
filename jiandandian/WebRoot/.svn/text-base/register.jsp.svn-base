...
<body>
	...
	<div class="container">
		<form action="register.jsp#" class="form-horizontal">
			...
			<div class="form-group">
				...
				<div class="col-sm-1">
					<img id="randomcodeimg"
						src="<%=request.getContextPath()%>/servlet/RandomCode"
						onclick="JavaScript:this.src='<%=request.getContextPath()%>/servlet/RandomCode?tm='+Math.random()"
						title="点击刷新">
				</div>
			</div>
			<div class="form-group">
				<label for="tel" class="col-sm-2 control-label">绑定手机</label>
				<div class="col-sm-9">
					<input id="phonenum" type="number" class="form-control" 
						placeholder="tel">
				</div>
				<div class="col-sm-1">
					<button id="sendMassage" class="btn btn-default">获取验证码</button>
				</div>
			</div>
	</div>
	<div class="form-group">
	...
	<script>
		...
		$("#sendMassage").click(function(){
			...
			$.get("http://127.0.0.1/<%=request.getContextPath()%>/servlet/SendMassage?phone="+phone+"");
			//alert();
		});
	</script>
